...
class ContentEncoder(nn.Module):   #new ,1d ,no connected layer
    def __init__(self, mel_d, kernel):
        ...
        self.norm_layer = nn.InstanceNorm1d(self.c_m1, affine=False)
...
class res_block_up(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsample:
            out = out + self.bn3(self.conv2(ori)) #####################
            #out = F.relu(out)
            #print('f(x):{}'.format(self.add_conv(ori).shape))
        else:
            out = out + ori
...
class Decoder(nn.Module):
    def __init__(self, mel_d, kernel):
        ...
        self.conv_last = nn.Conv1d(self.c_m+self.c_m, self.c_out, kernel_size=1) 
        ...
        self.head = nn.Sequential(
            ...
            res_block_up(self.c_m+64, self.c_m, self.kernel, self.stride),
            #Upsample(scale_factor=2),
            #PrintLayer(),
            res_block_up(self.c_m+self.c_m, self.c_m, self.kernel, self.stride),
            #Upsample(scale_factor=2),
            #PrintLayer(),
            res_block_up(self.c_m+self.c_m, self.c_m, self.kernel, self.stride),
            #Upsample(scale_factor=2),
    ...
    def forward(self, x, encoder_layer_outputs):
        ...
        skip_id=0
        for i in range(4):
            skip_fea=encoder_layer_outputs[-(skip_id+1)]
